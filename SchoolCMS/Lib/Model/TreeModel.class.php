...
class TreeModel
{
	...
	private $m_LinkedArrays;  //数据表的管理表3级
	...
	/*
		构造方法
	*/
	public function __construct($FileDataArray = array(), $LinkedArray = array(), $LinkedArrays = array())
	{
		/* 实例化公共文件，使用提示方法 */
		...
		$this->SetLinkedArrays($LinkedArrays);
	...
	*/
	...
	/*
		设置当前表的管理表数据3级
	*/
	private function SetLinkedArrays($LinkedArrays)
	{
		if(false == empty($LinkedArrays)) {
			$this->m_LinkedArrays = $LinkedArrays;
		} else {
			$this->m_LinkedArrays = array();
		}
	}
	...
	*/
	private function TreeDataLinkedDeleteInfo($DeleteValueID)
	{
		...
		if((false == empty($DeleteValueID)) && (false == is_array($DeleteValueID))) {
			...
			foreach($this->m_LinkedArray as $TableNameKey=>$LinkedValue) {
				if(false == empty($TableNameKey) && false == empty($LinkedValue)) {
					if((true == is_array($DataRet)) && (false == empty($DataRet))) {
						/* 实例化临时表对象 */
						$TreeDeleteObj = M("$TableNameKey");
						
						/* 循环所有节点的数据 */
						foreach($DataRet as $DeleteID) {
							/* 判断是否还有3级关联表 */
							if(false == empty($this->m_LinkedArrays)) {
								/* 判断当前表名在3级关联表中是否存在 */
								if(true == array_key_exists($TableNameKey, $this->m_LinkedArrays)) {
									//echo'<pre>';print_r($this->m_LinkedArrays[$TableNameKey]);exit();
									foreach($this->m_LinkedArrays[$TableNameKey] as $TableKey=>$TableValue) {
										$FileID = $TableValue['fileid'];
										$KeyDataArray = $TreeDeleteObj->field("$FileID")->where("$LinkedValue=$DeleteID")->select();
										/* 将查询到的数据给3级关联表方法处理 */
										$this->TreeLinkedDeletes($KeyDataArray, $FileID, $TableValue['tablename']);
									}
								}
							}
	...
	/*
		删除3级关联表里面的数据
	*/
	private function TreeLinkedDeletes($KeyDataArray = array(), $FileID = null, $FileTableName = null)
	{
		if(false == empty($KeyDataArray) && false == empty($FileID) && false == empty($FileTableName)) {
		//echo'<pre>';print_r($KeyDataArray);print_r($FileID);print_r($FileTableName);exit();
			/* 实例化临时3级表对象 */
			$TreeDeleteObjs = M("$FileTableName");
			/* 循环2级表的id去3级表里对应删除操作 */
			foreach($KeyDataArray as $DataValues) {
				$DeleteValue = $DataValues[$FileID];
				$Statu = $TreeDeleteObjs->where("$FileID=$DeleteValue")->delete();
			}
		}
	}
	...
	*/
	public function GetTreeDataHtml()
	{
		...
		foreach($DataArray as $DKey=>$DValue) {
			...
			/* 判断是否开启排序列 */
			if(($this->m_Sort == 1) && (false == empty($this->m_FileSort))) {
				$Html .= '<td width="38px" class="treetablecenter"><input type="text" size="1" value="'.$DValue[$this->m_FileSort].'" class="inputcommon input_out treemodelupdatesortclick treeinputsort" title="可编辑"/><span class="treedisplaynone">'.$DValue[$this->m_FileID].'</span></td>';
			}
		...
		if($PID == '-0-') {
			...
			if($this->m_Form == 1) {
				...
				$Html .= '<td><input type="submit" class="treemodelsubmit" name="treesubmit" value="提交" title="提交"></td>';
		...
		return $Javascript = '<style type="text/css">
					...
					function treeaddnode(id) {
						$(document).ready(function(){
							...
							if('.$this->m_Sort.' == 1) {
								html += "<td width=\'15px\'><input type=\'text\' size=\'1\' value=\'0\' name=\'treemodeladdsort[]\' class=\'inputcommon input_out treeinputsort\' /></td>";
							}
							...
							html += "<input type=\'text\'  name=\'treemodeladdname[]\' class=\'inputcommon input_out\'/><span class=\'treedelete\' title=\'移除\'>×</span></td><td></td></tr></table>";
					...
					$(document).ready(function(){
						...
						if('.$this->m_DbEditor.' == 1) {
							$(".treemodelupdatenamedblclick").live("dblclick", function() {
								...
								if(value == "null") value = "";
								$(this).html("<input class=\'inputcommon input_out treemodelnameinput\' value=\'"+value+"\' title=\'鼠标焦点移开后将会自动保存\'/><span class=\'treedisplaynone\'>"+valueid+"</span><span class=\'treedisplaynone\'>"+value+"</span>");
						...
						/* 单击编辑顺序 */
						$(".treemodelupdatesortclick").live("click", function() {
							$(this).attr("title", "鼠标焦点移开后将会自动保存");
						});
